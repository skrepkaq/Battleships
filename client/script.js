...
canvas.addEventListener("click", event => {
    ...
    if (
        ended & (x > field_startX + field_size + 25 - 60) &&
        x < field_startX + field_size + 25 + 60 &&
        y > field_startY + field_size + 30 - 15 &&
        y < field_startY + field_size + 30 + 15
    )
        return socket.send(
            JSON.stringify({
...
form.addEventListener("submit", e => {
    ...
    loginAlert.textContent = "Login must be 4-20 characters long, password - 6-50";
});

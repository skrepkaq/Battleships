...
def game_log(game, loser, winner = None):
    ...
    for player in game.get_players():
        if player == loser:
            insert('players', 'userId,gameId,board,result', (player.user.get_id(), game_id, player.board.get_base_board(), 0))
        elif player == winner:
            insert('players', 'userId,gameId,board,result', (player.user.get_id(), game_id, player.board.get_base_board(), 1))
        else:
...
def get_top(nick):
    ...
    JOIN games ON games.gameId = players.gameId
    WHERE users.userId = players.userId
        AND players.result = 0
        AND games.shots > 40) AS 'loses'
    FROM 'users'
    ORDER BY wins DESC,
            loses ASC;
    """)

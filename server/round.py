...
class Round():
    def __init__(self, game_type):
        ...
        self.players_list = []
        self.state = 0 #0 - wait for players #1 - place (#2 - wait_for_second pl) #3 - game #4 - end
        self.turn_index = 0
        self.shots = 0
        self.code = None
        if self.type == 0:
    ...
    async def finish_place(self, player, opponent):
        if self.state == 1 and not player.get_placed() and not player.board.ship_count() and player.board.get_ships() == [4, 3, 2, 1]:
            player.placed = True
            if opponent.get_placed(): #placing is finishd
                self.state = 3
